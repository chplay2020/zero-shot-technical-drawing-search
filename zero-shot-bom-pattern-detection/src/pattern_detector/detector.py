...
from pattern_detector.candidate_generator import Candidate, generate_candidates
...
from pattern_detector.geometry import aspect_ratio
...
class PatternDetector:
    ...
    def detect(self, pattern_bgr: np.ndarray, drawing_bgr: np.ndarray) -> InferenceResult:
        ...
        drawing_resized, scale = resize_max_side(drawing_bgr, self.config.max_image_side)
        ...
        query_ar = pattern_edges.shape[1] / float(pattern_edges.shape[0])

        for x, y, w, h, scale_c, rot_c in pruned:
            box = (x, y, w, h)
            chamfer = chamfer_score(
                pattern_edges, query_orient, dist_map, orient_map, box, self.config.num_orientation_bins
            )

            target_edges = drawing_edges[y : y + h, x : x + w]
            resized_query_edges = cv2.resize(pattern_edges, (w, h), interpolation=cv2.INTER_NEAREST)
            ...
            ar_score = aspect_ratio_score(query_ar, aspect_ratio(box))

            score = fuse_scores(chamfer, iou_score, dens_score, ar_score, self.config.chamfer_tau)
        ...
        vis = draw_detections(drawing_bgr, boxes, scores)
        runtime_ms = (time.time() - start) * 1000.0

        detections = [
            Detection(bbox=BoundingBox(x=b[0], y=b[1], w=b[2], h=b[3]), score=s)
